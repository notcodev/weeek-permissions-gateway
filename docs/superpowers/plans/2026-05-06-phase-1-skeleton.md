...
**Tech Stack:** Next.js 16 (App Router, Node runtime), TypeScript 6 (strict), pnpm, Drizzle ORM + PostgreSQL 16, Better Auth (email+password only this phase), tRPC v11 (stable) + TanStack Query v5 + superjson, Tailwind v4 + shadcn/ui (`tw-animate-css`), Zod v4, pino, vitest + testcontainers, ESLint 9 (flat config, pinned to 9.x because `eslint-config-next` v16 ships plugin versions that error on ESLint 10) + Prettier, GitHub Actions.
...
> Note: `tsconfig.tsbuildinfo` is generated by Next 16's incremental compile
> and must not be tracked.

> Note: TypeScript 6 will rewrite `tsconfig.json` on the first `next build`
> (changing `"jsx": "preserve"` → `"jsx": "react-jsx"` and adding
> `.next/dev/types/**/*.ts` to `include`). This is mandatory Next 16
> behaviour — accept it and move on.
...
- [ ] **Step 7: Write `eslint.config.mjs`** (flat config — Next 16 + ESLint 9)
...
`eslint-config-next` v16 default export **is the flat-config array** — spread
it rather than calling it. The package's bundled `eslint-plugin-react`,
`eslint-plugin-import`, and `eslint-plugin-jsx-a11y` versions are not
ESLint 10-compatible (the `scopeManager.addGlobals` API changed); pin
ESLint to `^9.39.4` until `eslint-config-next` lifts its plugin floor. The
extra `@typescript-eslint/*` rules from earlier drafts have been removed —
they require explicitly registering the `typescript-eslint` plugin, and
`eslint-config-next` already covers the important ones.
